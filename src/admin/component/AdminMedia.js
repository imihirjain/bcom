  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://bcom-backend.onrender.com/api/media/upload",
        formData,
        {
          headers: {
    ...
    <div className="ml-64 mt-24 container mx-auto p-6 bg-gray-50 min-h-screen rounded-lg shadow-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {mediaFiles.map((media) => (
          <div
            key={media._id}
            className="relative border border-gray-300 p-4 rounded-lg shadow-md bg-white hover:shadow-lg transition duration-300 ease-in-out"
          >
            {media.type === "image" ? (
              <img
                src={media.url}
